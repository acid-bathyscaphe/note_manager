...
def menu():
    ...
    def search_notes():
        note_to_show = [] #создаём список заметок которых нашли по поиску
        keyword = input('Введите ключевое слово (или оставьте пустым для продолжения):') # получаем ключевое слово и статус от пользователя
        status = input('Введите статус заметки (или оставьте пустым для продолжения):')
        if keyword == '' and status == '': #в зависимости от ввода совершаем поиск в базе данных
            print('поиск не совершился')
        elif keyword == '' and status != '':
            print('поиск по статусу')
            note_to_show = filter_notes_by_status(status,'notes.db')
        elif keyword != '' and status == '':
            print('поиск по ключевому слову')
            note_to_show = search_notes_by_keyword(keyword, 'notes.db')
        else:
            print('поиск по ключевому слову и статусу')
            note_to_show = search_notes_by_keyword(keyword, 'notes.db')
            note_to_show += filter_notes_by_status(status, 'notes.db')
            for note in note_to_show: #фильтруем заметки чтобы они не повторялись
                note_to_show_copy = note_to_show
                note_to_show_copy.remove(note)
                for second_note in note_to_show_copy:
                    if second_note['id'] == note['id']:
                        note_to_show.remove(second_note)
        if note_to_show != []:#если смогли найти заметки по запросу показываем выводим их в консоль
            display_notes(note_to_show)
        else:
            print('Заметки, соответствующие запросу, не найдены')#если нет, то сообщаем об этом и выходим из функции
        ...
        while True:
            ...
            if user_answer == 'да' and notes != []:
                ...
                if before_delite == len(notes):
                    print(
                        "Заметок с таким именем пользователя или заголовком не найдено")  # если размер списка не изменился, то сообщаем что такой заметки нет
                else:
                    print(
